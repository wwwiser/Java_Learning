...
* **Reference**
  * **参考框架**
    ...
    * [](https://github.com/Seazean/JavaNote/blob/main/Prog.md)
      ...
      > 以该笔记为基础框架，在此之上结合自己实际学习过程做一些修改
  
  * **参考笔记**
  
    * [](https://www.yuque.com/mo_ming/gl7b70/pxr4o7)
  
      > 可结合该笔记进行一些补充，会有一些视频内的代码demo可供参考
  
    * [](https://gitee.com/gu_chun_bo/java-construct/blob/master/java并发编程/java并发1.md)
  
  * **参考视频**
  
    * [](https://www.bilibili.com/video/BV16J411h7Rd/?vd_source=edfa1a737875ba478d7e699e1e975128)
...
* **线程**：线程是属于进程的，是一个基本的 CPU 执行单元，是程序执行流的最小单元。线程是进程中的一个实体，是系统 **独立调度的基本单位**，线程本身不拥有系统资源，只拥有一点在运行中必不可少的资源，与同属一个进程的其他线程共享进程所拥有的全部资源
  * 作用：使多道程序更好的并发执行，提高资源利用率和系统吞吐量，增强操作系统的并发性能
...
* 进程间通信较为复杂
  ...
  * 消息队列：内核中存储消息的链表，由消息队列标识符标识，能在不同进程之间提供 **全双工通信**，对比管道：
  ...
  2. 不同计算机之间的 **进程通信**，需要通过网络，并遵守共同的协议，例如 HTTP
...
> 1. 自行创建线程类，在线程类中重写 run 方法，即线程任务
>    * 直接继承 Thread 类 -> 单继承问题
>    * 实现 Runnable 接口 -> 更灵活
> 2. 调用匿名内部类写法，更为灵活
>    * 具体可参见内部类的优点
...
* **`start()` 方法底层其实是给 CPU 注册当前线程，并且触发 `run()` 方法执行**
  * **线程注册**：JVM 会向操作系统请求创建一个线程，并将该线程注册到线程调度器中。操作系统会为这个线程分配资源，并将其加入到线程调度队列中
  * **触发 `run()` 方法**：一旦线程被操作系统调度到 CPU 上执行，JVM 会调用线程的 `run()` 方法。`run()` 方法是线程的入口点，线程的实际逻辑通常定义在这个方法中
  * **底层机制**：`start()` 方法的底层实现依赖于操作系统和 JVM 的线程模型。在大多数现代操作系统中，线程的创建和调度是由操作系统的线程库（如 POSIX 线程库）完成的

* 线程的启动必须调用 `start()` 方法，如果线程直接调用 `run()` 方法，相当于变成了普通类的执行，此时主线程将只有执行该线程
  * 调用 `start()` 会启动一个新线程，`run()` 方法在新线程中执行
  * 直接调用 `run()` 方法不会启动新线程，`run()` 方法的代码会在当前线程中同步执行
...
public class ThreadDemo {
    public static void main(String[] args) {
		...
		t1.start();
        Thread t3 = new Thread(target);//Thread-0
    }
}
...
public class ThreadDemo {
    public static void main(String[] args) {
        ...
        //匿名内部类 + lambda表达式
        Thread t2 = new Thread(new FutureTask<String>(() -> Thread.currentThread().getName() + "->" + "Hello World"), "t2");
        t1.start();
        try {
...
> 可以将打断状态理解成背包内余粮，只要有余量（true），park 就失效
...
LockSupport 类在 同步 → park-unpark 详解
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-两阶段终止模式.png" style="zoom:80%;" />

打断线程可能在任何时间，所以需要考虑在任何时刻被打断的处理方法：

1. 利用 isInterrupted 方法
...
**协同式线程调度**：线程的执行时间由 **线程本身** 控制

* 优点：线程做完任务才通知系统切换到其他线程，相当于所有线程串行执行，不会出现线程同步问题
* 缺点：线程执行时间不可控，如果代码编写出现问题，可能导致程序一直阻塞，引起系统的奔溃

**抢占式线程调度**：线程的执行时间由 **系统** 分配

* 优点：线程执行时间可控，不会因为一个线程的问题而导致整体系统不可用
* 缺点：无法主动为某个线程多分配时间

Java 提供了 **线程优先级** 的机制，优先级会提示（hint）调度器优先调度该线程，但这仅仅是一个提示，调度器可以忽略它。在线程的就绪状态时，如果 CPU 比较忙，那么优先级高的线程会获得更多的时间片，但 CPU 闲时，优先级几乎没作用
...
> 《深入理解 java 虚拟机》也有部分涉及

内核级线程调度的成本较大，所以引入了更轻量级的协程。用户线程的调度由用户自己实现（多对一的线程模型，多 **个用户线程映射到一个内核级线程**），被设计为协同式调度，所以叫 **协程**

* **有栈协程**：协程会完整的做调用栈的保护、恢复工作，所以叫有栈协程
* **无栈协程**：本质上是一种有限状态机，状态保存在闭包里，比有栈协程更轻量，但是功能有限

有栈协程中有一种特例叫 **纤程**，在新并发模型中，一段纤程的代码被分为两部分，执行过程和调度器：
...
>
> <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-操作系统层面5种状态.png" style="zoom:80%;" />

线程由生到死的完整过程（生命周期）：当线程被创建并启动以后，既不是一启动就进入了执行状态，也不是一直处于执行状态，在 API 中 `java.lang.Thread.State` 这个枚举中给出了 **六种线程状态（java 中定义了 6 种）**：
...
![](https://gitee.com/wwwiser/typora-image/raw/master/JUC-线程6种状态.png)
...
**解决线程安全问题（避免临界区的竞态条件发生）**：

* **阻塞同步**：synchronized 关键字，lock 接口（JUC 包，JDK5 引入）

  > **互斥同步 -> 悲观的并发策略**

* **非阻塞式同步**：原子变量

  > **基于冲突检测的乐观并发策略 -> 依靠硬件指令集的发展（须要求操作和冲突检测这两个步骤具备原子性）**
  >
  > * **解释**：不管风险，先进行操作，如果没有其他线程争用共享数据，那操作就直接成功了；如果共享的数据的确被争用，产生了冲突，那再进行其他的补偿措施，最常用的补偿措施是不断地重试，直到出现没有竞争的共享数据为止
  >
  > **硬件指令集层面指令**
  >
  > * Test and Set
  > * Fetch and Increment
  > * Swap
  > * CAS(Compare and Swap) -> java 中暴露出来的指令
...
* 在 **实例方法** 中使用 this 作为锁对象，锁住的 this 正好是共享资源

* 在 **静态方法** 中使用类名 .class 字节码作为锁对象，因为静态成员属于类，被所有实例对象共享，所以需要锁住类

  > 也可以选取类级别的常量对象，eg. `static final Object LOCK = new Object();`
...
**继承性**：synchronized 修饰的方法的不具备继承性，所以子类是线程不安全的，如果子类的方法也被 synchronized 修饰，两个锁对象其实是一把锁，而且是 **子类对象作为锁**
...
* 锁住类对象，可以保证所有静态方法和静态变量的线程安全
* 锁住 this 对象，只有在当前实例对象的线程内是安全的，即当前实例对象的实例变量和实例方法，如果有多个实例就不安全
...
* **Mark Word 结构**：最后两位是 **锁标志位**
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-Monitor-MarkWord结构32位.png" style="zoom:80%;" />

  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-Monitor-MarkWord结构32位-2.png" style="zoom:80%;" />

* **64 位虚拟机 Mark Word**：

  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-Monitor-MarkWord结构64位.png" style="zoom:80%;" />
...
* 当 Thread-2 执行 synchronized(obj) 就会将 Monitor 的所有者 Owner 置为 Thread-2，Monitor 中只能有一个 Owner，**obj 对象的 Mark Word 指向 Monitor**，把 **对象原有的 MarkWord 存入线程栈中的锁记录** 中（轻量级锁部分详解）
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-Monitor工作原理1.png" style="zoom:80%;" />
* 在 Thread-2 上锁的过程，Thread-3、Thread-4、Thread-5 也执行 synchronized(obj)，就会进入 EntryList BLOCKED（双向链表）
* Thread-2 执行完同步代码块的内容，根据 obj 对象头中 Monitor 地址寻找，设置 Owner 为空，把线程栈的锁记录中的对象头的值设置回 MarkWord
* 唤醒 EntryList 中等待的线程来竞争锁，竞争是 **非公平的**，如果这时有新的线程想要获取锁，可能直接就抢占到了，阻塞队列的线程就会继续阻塞
* WaitSet 中的 Thread-0，是以前获得过锁，但条件不满足进入 WAITING 状态的线程（wait-notify 机制）

<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-Monitor工作原理2.png" style="zoom:80%;" />
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-锁升级过程.png" style="zoom:80%;" />

<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-锁升级过程2.png" style="zoom:80%;" />
...
**思想** 是偏向于让第一个获取锁对象的线程，这个线程之后重新获取该锁不再需要同步操作：
...
* 当一个对象已经计算过 hashCode，就再也无法进入偏向状态了 -> 是否生成一致性哈希码，即调用默认的 `hashcode()`

  > 1. **对象的哈希码**：
  >
  >    - **一致性哈希码**：默认的 `hashCode()` 实现返回的哈希码，它在对象的生命周期内保持不变
  >
  >      > 一旦计算过哈希码，它会永久存储在对象头中，不会改变
  >
  >    - **用户自定义哈希码**：如果用户重写了 `hashCode()` 方法，那么哈希码的值可以由用户控制，但 **强烈推荐保持哈希码的稳定性**，否则可能导致依赖哈希码的 API（如 `HashMap`、`HashSet` 等）出现错误
  >
  > 2. **偏向锁与哈希码的冲突**：
  >
  >    - 偏向锁状态和哈希码都存储在 Mark Word 中，因此它们不能同时存在
  >    - 如果对象已经计算了哈希码，它就无法进入偏向锁状态
  >    - 如果对象处于偏向锁状态，但需要计算哈希码，偏向锁会被撤销并膨胀为重量级锁
  >
  > 3. **重量级锁的哈希码存储**：
  >
  >    - 在重量级锁状态下，对象头的 Mark Word 指向 `ObjectMonitor` 对象
  >    - `ObjectMonitor` 对象中可以存储哈希码，从而解决哈希码与锁状态的冲突
...
**偏向锁优化策略**：如果对象被多个线程访问，但没有竞争

> 先尝试重偏向，再尝试撤销

* **批量重偏向（Bulk Rebiasing）**
  * 条件：当偏向锁的撤销次数超过一定阈值（例如 20 次）时，JVM 会认为当前的偏向策略可能不合理
  * 操作：JVM 会尝试将对象的锁重新偏向到新的加锁线程（例如 T2）。重偏向会重置对象的 `Thread ID`，使对象偏向新的线程
  * 目的：通过动态调整偏向锁的线程，减少锁的撤销次数，提高锁的效率

* **批量撤销（Bulk Revocation）**
  * 条件：当偏向锁的撤销次数超过更高阈值（例如 40 次）时，JVM 会认为偏向锁机制在这种场景下确实不合适
  * 操作：JVM 会将整个类的所有对象的锁状态改为 **不可偏向**（即禁用偏向锁）；新创建的对象也会默认为不可偏向
  * 目的：避免频繁的偏向锁撤销和重偏向操作，减少锁管理的开销
...
> **轻量级锁能提升程序同步性能的依据**：
>
> * 经验法则 -> 对于绝大部分的锁，在整个同步周期内都是不存在竞争
>
> * 如果没有竞争，轻量级锁便通过 CAS 操作成功避免了使用互斥量的开销；但如果确实存在锁竞争，除了互斥量的本身开销外，还额外发生了 CAS 操作的开销 -> 在有竞争的情况下，轻量级锁反而会比传统的重量级锁更慢

**使用条件**：一个对象有多个线程要加锁，但加锁的时间是错开的（没有竞争），可以使用轻量级锁来优化

> 轻量级锁对使用者是透明的（不可见）

**可重入锁**：线程可以进入任何一个它已经拥有的锁所同步着的代码块，可重入锁最大的作用是 **避免死锁**

> 可以理解成有一把钥匙，能够解锁用这个锁锁住的所有代码块，代码块内的代码不一定相同
...
* 创建锁记录（Lock Record）对象，每个线程的 **栈帧** 都会包含一个锁记录的结构，存储锁定对象的 Mark Word
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-轻量级锁原理1.png" style="zoom:80%;" />

* 让锁记录中 Object reference 指向锁住的对象，并尝试用 CAS 替换 Object 的 Mark Word，将 Mark Word 的值存入锁记录

* 如果 CAS 替换成功，对象头中存储了锁记录地址和状态 00（轻量级锁） ，表示由该线程给对象加锁
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-轻量级锁原理2.png" style="zoom:80%;" />
...
* 如果 CAS 失败，有两种情况：
  ...
  * 如果是线程自己执行了 synchronized 锁重入，就添加一条 Lock Record 作为重入的计数
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-轻量级锁原理3.png" style="zoom:80%;" />
...
* 当 Thread-1 进行轻量级加锁时，Thread-0 已经对该对象加了轻量级锁
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-重量级锁原理1.png" style="zoom:80%;" />

* Thread-1 加轻量级锁失败，进入锁膨胀流程：为 Object 对象申请 Monitor 锁，**通过 Object 对象头获取到持锁线程**，将 Monitor 的 Owner 置为 Thread-0，将 Object 的对象头指向重量级锁地址，然后自己进入 Monitor 的 EntryList BLOCKED

  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-重量级锁原理2.png" style="zoom:80%;" />
...
* **自旋成功**：
      <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-自旋成功.png" style="zoom:80%;" />

* **自旋失败**：

  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-自旋失败.png" style="zoom:80%;" />
...
**多把不相干的锁**：一间大屋子有两个功能睡觉、学习，互不相干。现在一人要学习，一人要睡觉，如果只用一间屋子（一个对象锁）的话，那么并发度很低 -> **提高并发度**
...
#### synchronized 关键词与 Lock 接口优化对比

##### **`synchronized` 的优化**

`synchronized` 是 Java 语言内置的同步机制，它的实现与 JVM 的底层机制紧密集成。当使用 `synchronized` 时，JVM 可以利用线程和对象的元数据来记录锁的状态，从而实现高效的锁优化

1. **锁的轻量级实现**

- **偏向锁（Biased Locking）**：当一个线程第一次获取锁时，JVM 会将锁偏向该线程。如果后续该线程再次获取锁，无需进行复杂的同步操作，从而减少锁的开销
- **轻量级锁（Lightweight Locking）**：当多个线程都要使用锁时，JVM 会尝试使用轻量级锁，通过 CAS（Compare-And-Swap）操作来避免重量级的互斥锁（如操作系统级别的互斥锁）
- **自旋锁（Spin Lock）**：在某些情况下，JVM 会使用自旋锁来减少线程阻塞的开销。自旋锁允许线程在等待锁释放时进行短暂的自旋（循环等待），而不是直接进入阻塞状态

2. **锁的升级机制**

JVM 会根据锁的使用情况动态调整锁的级别：

- **偏向锁 → 轻量级锁 → 重量级锁**：如果锁被频繁竞争，JVM 会逐步升级锁的级别，以适应不同的场景

3. **锁的释放和回收**

JVM 可以利用线程和对象的元数据来记录锁的状态，例如：

- **锁的持有者**：记录哪个线程持有锁
- **锁的释放**：当线程退出同步块时，JVM 可以自动释放锁

------

##### **J.U.C 中的 `Lock`**

`java.util.concurrent.locks.Lock`（如 `ReentrantLock`）是基于 AQS（AbstractQueuedSynchronizer）实现的锁机制。它提供了比 `synchronized` 更灵活的锁功能，但同时也带来了以下特点：

1. **锁的实现基于 Java 代码**

`Lock` 的实现是基于 Java 代码的，而不是直接依赖于 JVM 的底层机制。这意味着：

- JVM 无法直接感知 `Lock` 的状态，例如哪些线程持有锁、锁的持有次数等
- `Lock` 的实现需要通过显式的锁操作（如 `lock()` 和 `unlock()`）来管理锁的状态

2. **锁的性能优化**

虽然 `Lock` 提供了更灵活的功能（如可中断锁、条件变量等），但它的性能优化主要依赖于 Java 代码层面的实现，而不是 JVM 的底层优化。例如：

- `ReentrantLock` 使用 AQS 的队列机制来管理线程的等待和唤醒
- 自旋锁和锁的释放需要显式地通过代码实现
...
* **原理** 不同：sleep() 方法是属于 Thread 类，是线程用来控制自身流程的，使此线程暂停执行一段时间而把执行机会让给其他线程；wait() 方法属于 Object 类，用于线程间通信
* 对 **锁的处理机制** 不同：调用 sleep() 方法的过程中，线程不会释放对象锁，当调用 wait() 方法的时候，线程会放弃对象锁，进入等待此对象的等待锁定池（不释放锁其他线程怎么抢占到锁执行唤醒操作），但是都会释放 CPU
* **使用区域** 不同：wait() 方法必须放在 **同步控制方法和同步代码块（先获取锁）** 中使用，sleep() 方法则可以放在任何地方使用
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-park原理1.png" style="zoom:80%;" />
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-park原理2.png" style="zoom:80%;" />
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-保护性暂停.png" style="zoom:80%;" />
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-保护性暂停多任务版.png" style="zoom:80%;" />
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-生产者消费者模式.png" style="zoom:80%;" />
...
根据 JMM 的设计，系统存在一个主内存（Main Memory），Java 中所有变量都存储在主存中，对于所有线程都是共享的；每条线程都有自己的工作内存（Working Memory），工作内存中保存的是主存中某些 **变量的拷贝**，线程对所有变量的操作都是先对变量进行拷贝，然后在工作内存中进行，不能直接操作主内存中的变量；不同的线程之间也无法直接访问对方工作内存中的变量，线程间变量值的传递均需要通过主内存来完成，线程间的通信（传递）必须通过主内存来完成

<img src="https://gitee.com/wwwiser/typora-image/raw/master/JMM内存模型.png" style="zoom:80%;" />
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JMM-内存交互.png" style="zoom:80%;" />
...
**存在不可见问题的根本原因**：由于缓存的存在，线程持有的是共享变量的副本，无法感知其他线程对于共享变量的更改，导致读取的值不是最新的。但是 final 修饰的变量是 **不可变** 的，就算有缓存，也不会存在不可见的问题
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JMM-可见性例子.png" style="zoom:80%;" />
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JMM-CPU缓存结构.png" style="zoom:80%;" />
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-内存伪共享.png" style="zoom:80%;" />
...
* 读屏障（lfence，Load Barrier）保证在该屏障之后的，对共享变量的读取，从主存刷新变量值，加载的是主存中最新数据
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JMM-volatile保证可见性.png" style="zoom:80%;" />
...
* 有序性的保证也只是保证了本线程内相关代码不被重排序
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JMM-volatile不能保证原子性.png" style="zoom:80%;" />
...
* 第一个 if 使用了 INSTANCE 变量，是在同步块之外，但在多线程环境下会产生问题
  ...
  > INSTANCE 已赋值，但对象却为完全初始化完成，此时多线程环境中调用对象会出现问题
...
> 设置写屏障，保证 INSTANCE 赋值前，初始化已完成并更新到主存中，即初始化操作必定发生在赋值之前
...
1. 程序次序规则 (Program Order Rule)：在一个线程内，按照控制流顺序，书写在前面的操作先行发生于书写在后面的操作
   ...
   > 一个线程内，逻辑上书写在前面的操作先行发生于书写在后面的操作 ，因为多个操作之间有先后依赖关系，则不允许对这些操作进行重排序

2. 管程锁定规则 (Monitor Lock Rule)：一个 unlock 操作先行发生于后面（时间的先后）对同一个锁的 lock 操作，所以线程解锁 m 之前对变量的写（解锁前会刷新到主内存中），对于接下来对 m 加锁的其它线程对该变量的读可见
...
> JDK 9 之后引入了 VarHandle，新版本 JDK 源码中很多部分使用 VarHandle 替代了 Unsafe，但是分析思路基本一致
...
//其他基本数据类型(long,char,float,double,short等)的操作与putLong及getLong相同
...
//其他基本数据类型(long,char,byte,float,double)的操作与putIntVolatile及getIntVolatile相同，引用putObjectVolatile也一样。

//与putIntVolatile一样，但要求被操作字段必须有volatile修饰，具有延迟赋值效果
...
将一个线程进行挂起是通过 park 方法实现的，调用 park 后，线程将一直阻塞直到超时或者中断等条件出现。unpark 可以终止一个挂起的线程，使其恢复正常

Java 对线程的挂起操作被封装在 LockSupport 类中，LockSupport 类中有各种版本 park 方法，其底层实现最终还是使用 Unsafe.park()方法和 Unsafe.unpark()方法
...
包括了 loadFence、storeFence、fullFence 等方法，这些方法是在 Java 8 新引入的，用于定义内存屏障，避免代码重排序
...
参考网址：[](https://blog.csdn.net/javazejian/article/details/72772470)
...
> JDK9 之后陆续用 VarHandle 去替代 Unsafe
>
> 源码分析部分不适用于新版本，但是分析思路依旧适用

#### 原子基本类型

**常见原子类**：AtomicInteger、AtomicBoolean、AtomicLong

**原理分析**：自旋锁  + CAS 算法 -> 调用 Unsafe 类 CAS 操作相关方法，具体参见 6.2.2.5

> 以 AtomicInteger 为例，其余两个类似
...
| `public final int getAndUpdate(IntUnaryOperator updateFunction)` | 以原子方式按照输入的更新方式去操作实例中的值并返回           |
...
public class AtomicInteger extends Number implements java.io.Serializable {
    ...
    //按照updateFunction更新方式去操作旧值，返回旧值，底层CAS操作
    ...
    /*
    IntUnaryOperator为函数式接口可以使用lambda表达式
...
> 从源码看来，AtomicReference 与 AtomicInteger 的实现原理基本是一样的，都是调用 Unsafe 类 CAS 操作相关方法，具体参见 6.2.2.5
...
> 以 AtomicIntegerArray 为例，其余两个类似
...
**作用**：如果我们只需要某个类里的某个字段，也就是说让普通的变量也享受原子操作，可以使用原子更新字段类

>  如在某些时候由于项目前期考虑不周全，项目需求又发生变化，使得某个类中的变量需要执行多线程操作，由于该变量多处使用，改动起来比较麻烦，而且原来使用的地方无需使用线程安全，只要求新场景需要使用时，可以借助原子更新器处理这种场景

**使用条件**：

1. 操作的字段不能是 static 类型

2. 操作的字段不能是 final 类型的，因为 final 根本没法修改

3. 字段必须是 volatile 修饰的，也就是数据本身是读一致的

4. 属性必须对当前的 Updater 所在的区域是可见的，如果不是当前类内部进行原子更新器操作不能使用 private，protected 子类操作父类时修饰符必须是 protect 权限及以上，如果在同一个 package 下则必须是 default 权限及以上，也就是说无论何时都应该保证操作类与被操作类间的可见性
...
> 以 LongAdder 为例进行分析，LongAdder 和 DoubleAdder 实现原理大同小异
...
* LongAdder 类是 LongAccumulator 类的一个特例，只是 LongAccumulator 提供了更强大的功能，可以自定义累加规则，当 accumulatorFunction 为 null 时就等价于 LongAddr
...
> **缓存行(cashe line)**：
>
> * 定义：缓存行是 CPU 缓存系统中存储数据的基本单位
>
>   > 缓存行是内存中一段连续的字节序列，长度通常是 2 的幂，比如 32 字节、64 字节或 256 字节 
>   >
>   > 最常见的缓存行大小是 64 字节
>
> * 作用：缓存行是缓存与内存之间数据传输的最小单元
>
>   > 当 CPU 从内存中读取数据时，它会以缓存行的大小为单位将数据加载到缓存中
>   >
>   > 当缓存中的数据被修改后，也会以缓存行的大小为单位写回到内存
>
> 
>
> **CPU 缓存的写策略**：
>
> * **Write-Through**：当 CPU 修改缓存中的数据时，同时也会将修改写入内存。这种策略的优点是内存中的数据始终是最新的，但缺点是写操作的性能较低，因为每次写操作都需要访问内存。
>
>   > L1 缓存采取
>
> * **Write-Back**：当 CPU 修改缓存中的数据时，不会立即写入内存，而是将修改标记为 Dirty（脏）。只有当缓存行被替换出缓存时，才会将修改写回到内存。这种策略的优点是减少了内存的写操作次数，提高了性能，但缺点是可能导致缓存与内存之间的数据不一致。
>
>   > L2、L3 缓存采取
>
> 
...
> **多处理器环境中的缓存一致性问题**：
>
> 在多处理器（MP）系统中，每个 CPU 都有自己的缓存，这些缓存是私有的。因此，不同 CPU 缓存中的数据可能会出现不一致的情况。为了解决这个问题，现代多处理器架构（如 CC-NUMA 和 SMP）实现了 缓存一致性机制（Cache Coherence Mechanism）。
>
> > **Cache-Snooping 协议**：这是实现缓存一致性的一种常见方法。每个 CPU 通过监听总线（Bus）上的操作来监控其他 CPU 对缓存的读写行为
> >
> > * 写操作：
> >   * 当 CPU1 写入缓存时，其他 CPU 会检查自己缓存中对应的缓存行
> >   * 如果该缓存行是 Dirty（脏）的，则将其写回到内存，并将 CPU1 的相关缓存行刷新
> >   * 如果该缓存行不是 Dirty 的，则将其标记为 Invalid（无效）
> > * 读操作：
> >   * 当 CPU1 读取缓存时，其他 CPU 会将自己缓存中对应的缓存行中标记为 Dirty 的部分写回到内存，并将 CPU1 的相关缓存行刷新



**伪共享（False Sharing）**：

* 定义：当多个 CPU 核心并行运行的进程或线程同时读写同一个缓存行时，即使它们访问的是缓存行中不同的字段，也会导致缓存行在不同 CPU 缓存之间频繁地同步和刷新

* 后果：产生缓存行震荡（Cache Thrashing）

  > **缓存行震荡（Cache Thrashing）**：这种频繁的同步和刷新会导致缓存与内存之间的大量数据传输，从而显著降低系统性能
  >
  > **解决方式**：缓存行对齐 -> 缓存行对齐是指将数据的存储位置调整为缓存行大小的整数倍，以便 CPU 可以一次性加载整个数据 -> 确保不同线程访问的变量位于不同的缓存行中，从而减少缓存行的争用和同步开销
  >
...
//JDK11之后，位置移动到@jdk.internal.vm.annotation.Contended
...
* **Cell 存储形式**：Cell 是数组形式，**在内存中是连续存储的**，64 位系统中，一个 Cell 为 24 字节（16 字节的对象头和 8 字节的 value），每一个 cache line 为 64 字节，因此缓存行可以存下 2 个的 Cell 对象，当 Core-0 要修改 Cell [0]、Core-1 要修改 Cell [1]，无论谁修改成功都会导致当前缓存行失效，从而导致对方的数据失效，需要重新去主存获取，影响效率

<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-伪共享1.png" style="zoom:80%;" />

@sun.misc.Contended：防止缓存行伪共享，在使用此注解的对象或字段的前后各增加 128 字节大小的 padding，使用 2 倍于大多数硬件缓存行让 CPU 将对象预读至缓存时 **占用不同的缓存行**，这样就不会造成对方缓存行的失效

> ```java
> //jdk6
> //Long Padding：七个long进行padding，8B(对象头) + 7 * 8B = 64B
> Java view:  
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-伪共享2.png" style="zoom:80%;" />
...
> JDK9 之后引入 VarHandle 之后，源码也发生了改变，但分析思路一致
...
* Striped64 的设计核心思路就是通过内部的分散计算来避免竞争
  ...
  * 没有竞争的情况下，要累加的数通过 cas 累加到 base 上
  * 如果有竞争的话，会将要累加的数累加到 Cells 数组中的某个 cell 元素里面
  * 整个 Striped64 的值为 sum = base+∑ [0~n] cells
...
* LongAdder 则有两个值用于累加 -> sum = base+∑ [0~n] cells
  * base，它的作用类似于 AtomicInteger 里面的 value，在没有竞争的情况不会用到 cells 数组，它为 null，这时使用 base 做累加
  * 有了竞争后 cells 数组就上场了，第一次初始化长度为 2，以后每次扩容都是变为原来的两倍，直到 cells 数组的长度大于等于当前服务器 cpu 的数量为止就不在扩容（硬件限制，即使扩容也并不能有效）；每个线程会通过线程对 cells [threadLocalRandomProbe%cells.length] 位置的 Cell 对象中的 value 做累加，这样相当于将线程绑定到了 cells 中的某个 cell 对象上

**cellsBusy**：

* cellsBusy，它有两个值 0 或 1，它的作用是当要修改 cells 数组时加锁，防止多线程同时修改 cells 数组，0 为无锁，1 为加锁

  > 加锁的状况 -> 对 cells 数组进行修改时

  1. cells 数组 **初始化** 的时候；
  2. cells 数组 **扩容** 的时候；
  3. 如果 cells 数组中某个元素为 null，给这个位置 **创建新的 Cell 对象** 的时候；
...
* `Striped64::getProbe`：获取当前线程 threadLocalRandomProbe 的值
...
```java
//Thread类的成员变量threadLocalRandomProbe
//LongAdder用来hash定位Cells数组位置的，平时线程的这个变量一般用不到，它的值一直都是0
@sun.misc.Contended("tlr")
    int threadLocalRandomProbe;
```

* `ThreadLocalRandom::current`：获取当前 threadLocalRandomProbe 的值，如果为 0，则初始化 threadLocalRandomProbe

> 线程对 LongAdder 的累加操作，在没有进入 longAccumulate 方法前，threadLocalRandomProbe 一直都是 0，当发生争用后才会进入 longAccumulate 方法中，进入该方法第一件事就是判断 threadLocalRandomProbe 是否为 0，如果为 0，则调用 `ThreadLocalRandom::current`
>
...
//probeGenerator是static类型的AtomicInteger类，每执行一次localInit()方法，都会将probeGenerator累加一次0x9e3779b9这个值
//0x9e3779b9这个数字的得来是 2^32 除以一个常数，这个常数就是传说中的黄金比例 1.6180339887
//将当前线程的threadLocalRandomProbe设置为probeGenerator的值，如果probeGenerator为0，则取1；
...
* `Striped64::advanceProbe`：threadLocalRandomProbe 重新生成
...
> 这个类很简单，final 类型，内部有一个 value 值，使用 cas 来更新它的值
>
> Cell 类唯一需要注意的地方就是 Cell 类的注解@sun.misc.Contended -> 用来解决伪共享问题，具体分析参见 6.4.3
...
* `LongAdder::add`：累加方法
  ...
  > 保证 cells 惰性加载，优先尝试 casBase，即直接更新 base 域
...
* `Striped64::longAccumulate`：cell 数组创建
  ...
  >     * CASE 1.6: 【扩容逻辑】，进行加锁
  >
  >       CASE 1.7: 重置当前线程 Hash 值，开始下一次尝试，这就是【分段迁移机制】
  >
              ...
              if ((a = as[(n - 1) & h]) == null) {
                  ...
                  if (cellsBusy == 0) {   
                      ...
                      if (cellsBusy == 0 && casCellsBusy()) {
                          ...
                          if (created)			// true 表示创建完成，可以退出循环了
                              break;
                          continue;
                      }
                  }
...
参考网址：[](https://blog.csdn.net/zqz_zqz/article/details/70665941)
...
**解决方式**：AtomicStampedReference、AtomicMarkableReference



**AtomicStampedReference**：

> AtomicStampedReference 原子类是一个 **带有时间戳的对象引用**，在每次修改后，AtomicStampedReference 不仅会设置新值而且还会记录更改的时间
>
> 当 AtomicStampedReference 设置对象值时，**对象值以及时间戳都必须满足期望值** 才能写入成功

* **实现思想**：通过一个键值对 Pair 存储数据和时间戳，在更新时对数据和时间戳进行比较，只有两者都符合预期才会调用 Unsafe 的 compareAndSwapObject 方法执行数值和时间戳替换，也就避免了 ABA 的问题
...
* **常用 API**：
  ...
  * `public void set(V newReference, int newStamp)`：设置值和版本号 xxfc
...
​	`AtomicStampedReference::compareAndSet`：同时对当前数据和当前时间进行比较，只有两者都相等是才会执行 casPair()方法
...
​	`AtomicStampedReference::casPair`：casPair 最终调用的还是 Unsafe 类中的 compareAndSwapObject 方法
...
> AtomicMarkableReference 维护的是一个 boolean 值的标识，也就是说至于 true 和 false 两种切换状态
>
> 这种方式并不能完全防止 ABA 问题的发生，**只能减少 ABA 问题发生的概率**
>
> 实现原理同 AtomicStampedReference 类似，不再展开
...
ThreadLocal 类用来提供线程内部的局部变量，这种变量在多线程环境下访问（通过 get 和 set 方法访问）时能保证各个线程的变量相对独立于其他线程内的变量，有可能分配在堆内的 **TLAB** 中

> **TLAB（Thread Local Allocation Buffer）**
>
> **TLAB** 是 Java 虚拟机（JVM）中用于优化对象分配的一种机制，与 `ThreadLocal` 完全不同。
>
> **TLAB 的工作原理**
>
> - **目的**：减少多线程环境下的内存分配竞争，提高对象分配的性能。
> - **机制**：JVM 为每个线程分配一个独立的内存区域（TLAB），线程在创建对象时优先使用自己的 TLAB，从而减少对共享堆内存的竞争。
> - **存储位置**：TLAB 是 JVM 在堆内存中为每个线程分配的临时缓冲区，用于快速分配小对象。
> - **适用场景**：主要用于优化频繁创建小对象的场景，减少锁竞争和内存分配的开销。
>
> **TLAB 的配置**
>
> - JVM 提供了一些参数用于控制 TLAB 的大小和行为，例如：
...
* **传递数据**：通过 ThreadLocal 实现在同一线程不同函数或组件中传递公共变量，减少传递复杂度
  ...
  > - **减少参数传递**：通过 `ThreadLocal`，可以在同一个线程的不同方法或不同组件（如不同的类或模块）之间共享数据，而无需显式地将这些数据作为参数传递
  > - **简化代码**：减少了参数传递的复杂度，使代码更简洁、更易于维护。
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ThreadLocal数据结构JDK8前.png" style="zoom:80%;" />
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ThreadLocal数据结构JDK8后.png" style="zoom:80%;" />
...
* `set()`：添加数据，ThreadLocalMap 使用 **线性探测法来解决哈希冲突**
  ...
  * 在探测过程中 ThreadLocal 会复用 key 为 null 的脏 Entry 对象，并进行垃圾清理，防止出现内存泄漏
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-replaceStaleEntry流程.png" style="zoom:80%;" />
...
* **探测式清理**：沿着开始位置向后探测清理过期数据，沿途中碰到未过期数据则将此数据 rehash 在 table 数组中的定位，重定位后的元素理论上更接近 `i = entry.key & (table.length - 1)`，让 **数据的排列更紧凑**，会优化整个散列表查询性能
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ThreadLocal探测式清理1.png" style="zoom:80%;" />

  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ThreadLocal探测式清理2.png" style="zoom:80%;" />
...
* **如果 key 使用强引用**：使用完 ThreadLocal ，threadLocal Ref 被回收，但是 threadLocalMap 的 Entry 强引用了 threadLocal，造成 threadLocal 无法被回收，无法完全避免内存泄漏
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ThreadLocal内存泄漏强引用.png" style="zoom:80%;" />
...
* **如果 key 使用弱引用**：使用完 ThreadLocal ，threadLocal Ref 被回收，ThreadLocalMap 只持有 ThreadLocal 的弱引用，所以 threadlocal 也可以被回收，此时 Entry 中的 key = null。但没有手动删除这个 Entry 或者 CurrentThread 依然运行，依然存在强引用链，value 不会被回收，而这块 value 永远不会被访问到，也会导致 value 内存泄漏
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ThreadLocal内存泄漏弱引用.png" style="zoom:80%;" />

> ThreadLocalMap 中使用的 key 是 ThreadLocal 的弱引用，弱引用的特点是，如果这个对象只存在弱引用，那么在下一次垃圾回收的时候必然会被清理掉
>
...
> 交给 ThreadLocalMap 中的探测式清理和启发式清理来处理，具体可参见 6.8.4.3
...
* 当一个节点入队：
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-LinkedBlockingQueue入队流程.png" style="zoom:80%;" />
...
* `h = head` → `first = h.next` 
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-LinkedBlockingQueue出队流程1.png" style="zoom:80%;" />

* `h.next = h` → `head = first`

  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-LinkedBlockingQueue出队流程2.png" style="zoom:80%;" />
...
1. 线程 put1 执行 put(1)操作，由于当前没有配对的消费线程，所以 put1 线程入栈，自旋一小会后睡眠等待

<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-TransferStack实现-1.png" style="zoom:80%;" />

2. 接着，线程 put2 再次执行了 put(2)操作，跟前面一样，put2 线程入栈，自旋一小会后睡眠等待

<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-TransferStack实现-2.png" style="zoom:80%;" />
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-TransferStack实现-3.png" style="zoom:80%;" />

4. 最后，再来一个线程 take2，执行 take 操作，这跟步骤 3 的逻辑基本是一致的，take2 线程入栈，然后在循环中匹配 put1 线程，最终全部匹配完毕，栈变为空，恢复初始状态

<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-TransferStack实现-4.png" style="zoom:80%;" />
...
1. 初始化时，TransferQueue 的状态

<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-TransferQueue实现-1.png" style="zoom:80%;" />

2. 线程 put1 执行 put(1)操作，由于当前没有配对的消费线程，所以 put1 线程入队列，自旋一小会后睡眠等待

<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-TransferQueue实现-2.png" style="zoom:80%;" />

3. 接着，线程 put2 执行了 put(2)操作，跟前面一样，put2 线程入队列，自旋一小会后睡眠等待

<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-TransferQueue实现-3.png" style="zoom:80%;" />

4. 这时候，来了一个线程 take1，执行了 take 操作，由于 tail 指向 put2 线程，put2 线程跟 take1 线程配对了(一 put 一 take)，这时 take1 线程不需要入队，要唤醒的线程并不是 put2，而是 put1，执行后 put1 线程被唤醒，take1 线程的 take()方法返回了 1(put1 线程的数据)，这样就实现了线程间的一对一通信

   > 即队列中的元素必定状态一致，不然必然会出队

<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-TransferQueue实现-4.png" style="zoom:80%;" />

5. 最后，再来一个线程 take2，执行 take 操作，这时候只有 put2 线程在等候，而且两个线程匹配上了，线程 put2 被唤醒，take2 线程 take 操作返回了 2(线程 put2 的数据)，这时候队列又回到了起点

<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-TransferQueue实现-5.png" style="zoom:80%;" />
...
参考网址：[](https://blog.csdn.net/yanyan19880509/article/details/52562039)
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-线程池工作原理.png" style="zoom:80%;" />
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-newSingleThreadExecutor.png" style="zoom:80%;" />
...
* 状态表示：
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-线程池状态转换图.png" style="zoom:80%;" />
...
* Fork/Join 使用 ForkJoinPool 来启动，是一个特殊的线程池，默认会创建与 CPU 核心数大小相同的线程池
...
* 请求的共享资源被占用，AQS 用队列实现线程阻塞等待以及被唤醒时锁分配的机制，将暂时获取不到锁的线程加入到队列中
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-AQS原理图.png" style="zoom:80%;" />
...
* 使用了 FIFO 先入先出队列，并不支持优先级队列，**同步队列是双向链表，便于出队入队**
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-AQS队列设计.png" style="zoom:80%;" />
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ReentrantLock-非公平锁1.png" style="zoom:80%;" />
...
* 接下来进入 addWaiter 逻辑，构造 Node 队列（不是阻塞队列），前置条件是当前线程获取锁失败，说明有线程占用了锁
  ...
  * Node 的创建是懒惰的，其中第一个 Node 称为 **Dummy（哑元）或哨兵**，用来占位，并不关联线程
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ReentrantLock-非公平锁2.png" style="zoom:80%;" />
...
* 再有多个线程经历竞争失败后：
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ReentrantLock-非公平锁3.png" style="zoom:80%;" />
...
* 唤醒的线程会从 park 位置开始执行，如果加锁成功（没有竞争），会设置
  ...
  * 原本的 head 因为从链表断开，而可被垃圾回收（图中有错误，原来的头节点的 waitStatus 被改为 0 了）
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ReentrantLock-非公平锁4.png" style="zoom:80%;" />
...
* 如果这时有其它线程来竞争 **（非公平）**，例如这时有 Thread-4 来了并抢占了锁
  ...
  * Thread-1 再次进入 acquireQueued 流程，获取锁失败，重新进入 park 阻塞
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ReentrantLock-非公平锁5.png" style="zoom:80%;" />
...
public final boolean hasQueuedPredecessors() {    
    ...
    return h != t &&
        ...
        // 判断原因：1.防止出现空指针异常; 2.保证线程安全，防止其他线程在进行unparkSuccessor时短暂出现head.next = null并且head != tail的中间状态造成误判
...
* 开始 Thread-0 持有锁，调用 await，线程进入 ConditionObject 等待，直到被唤醒或打断，调用 await 方法的线程都是持锁状态的，所以说逻辑里 **不存在并发**
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ReentrantLock-条件变量1.png" style="zoom:80%;" />
...
* fullyRelease 中会 unpark AQS 队列中的下一个节点竞争锁，假设 Thread-1 竞争成功
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ReentrantLock-条件变量2.png" style="zoom:80%;" />
...
* 执行 transferForSignal，**先将节点的 waitStatus 改为 0，然后加入 AQS 阻塞队列尾部**，将 Thread-3 的 waitStatus 改为 -1
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ReentrantLock-条件变量3.png" style="zoom:80%;" />
...
* 获取读锁失败，进入 sync.doAcquireShared(1) 流程开始阻塞，首先也是调用 addWaiter 添加节点，不同之处在于节点被设置为 Node.SHARED 模式而非 Node.EXCLUSIVE 模式，注意此时 t2 仍处于活跃状态
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ReentrantReadWriteLock加锁1.png" style="zoom:80%;" />

* 这种状态下，假设又有 t3 r.lock，t4 w.lock，这期间 t1 仍然持有锁，就变成了下面的样子

  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ReentrantReadWriteLock加锁2.png" style="zoom:80%;" />
...
* 接下来 t2 调用 setHeadAndPropagate(node, 1)，它原本所在节点被置为头节点；还会检查下一个节点是否是 shared，如果是则调用 doReleaseShared() 将 head 的状态从 -1 改为 0 并唤醒下一个节点，这时 t3 在 doAcquireShared 内 parkAndCheckInterrupt() 处恢复运行，**唤醒连续的所有的共享节点**
  ...
  private void setHeadAndPropagate(Node node, int propagate) {
      ...
      // h == null 以及 (h = head) == null 都是用来防止出现空指针异常，只要为 null 直接进入 if 内语句
      if (propagate > 0 || h == null || h.waitStatus < 0 ||
          (h = head) == null || h.waitStatus < 0) {
          // 获取下一个节点
          Node s = node.next;
          // 如果当前是最后一个节点，或者下一个节点是【等待共享读锁的节点】
          // s == null 用来防止出现空指针异常，只要为 null 直接进入 if 内语句
      ...
      for (;;) {
          ...
          if (h != null && h != tail) {
              ...
              if (ws == Node.SIGNAL) {
                  ...
                  // 其他线程释放读锁时有可能会调用 doReleaseShared 方法
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ReentrantReadWriteLock解锁1.png" style="zoom:80%;" />
...
* t4 在 acquireQueued 中 parkAndCheckInterrupt 处恢复运行，再次 for (;;) 这次自己是头节点的临节点，并且没有其他节点竞争，tryAcquire(1) 成功，修改头结点，流程结束
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ReentrantReadWriteLock解锁2.png" style="zoom:80%;" />
...
2. StampedLock 没有使用AQS 同步框架，而是完全自己实现的同步状态 state 和 CLH队列维护算法
3. 同步状态`state`的低7位标识读锁的数量，第8位标识写锁是否被占用，高24位记录写锁的版本，每次释放写锁会版本位置会加1
4. 写锁每次获取state会加128，释放也会加128，读锁是加减1
5. StampedLock 的连续多个读锁线程，只有第一个是在队列上，后面的读线程都存在第一个线程的cowait栈结构上
6. StampedLock 唤醒一个读锁线程后，读锁线程会唤醒所有在它`cowait`栈上的等待读锁线程
...
**StampedLock 与 ReentrantReadWriteLock的区别**
...
6. StampedLock 唤醒一个读锁线程后，读线程会唤醒所有在它`cowait`栈上的等待读锁线程
...
StampedLock中的连续申请的读锁节点通过cowaiter新起了一个维度链表来存放



<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-StampedLock CLH队列.png" style="zoom:80%;" />
...
private static final long WBIT  = 1L << LG_READERS;// 写锁的标识位 1000 0000 -> 128
private static final long RBITS = WBIT - 1L;// 读状态标识 0111 1111 -> 127
private static final long RFULL = RBITS - 1L;// 读锁的最大标识 0111 1110 -> 126
private static final long ABITS = RBITS | WBIT;// 用来读取读写状态，即掩码 1111 1111 -> 255
private static final long SBITS = ~RBITS; // 1 ~ 1(24位) 1000 0000, note overlap with ABITS, stamp数量上限

// 同步状态state的初始值 0 ~ 0(23位) 1 0000 0000
...
// 同步状态 初始值 256 0001 0000 0000，即 ORIGIN
private transient volatile long state;
//	int	 ->	|	  stamp 24位	       |   0无锁，1有锁 1位   |  悲观读锁数 |
// state -> 000000000000000000000000          0          0 0 0 0 0 0 0
...
WNode内部类：用于维护 CLH 队列的节点

```java
static final class WNode {
    volatile WNode prev;// 前驱节点
    volatile WNode next;// 后继节点
    volatile WNode cowait;// 悲观读锁对应的栈 
    volatile Thread thread;// 对应的线程
    volatile int status;// 节点的状态：取消CANCELLED 1、等待WATING -1   
    final int mode;// 节点类型：读锁、写锁
...
public long tryOptimisticRead() {
    ...
    // WBIT = 128, SBITS = ~RBITS(127), SBITS 的后 7 位为 0 ，其他都为 1
    // s & SBITS 就是我们前 25 位，版本号+锁标志(0)
    // state & WBIT ！= 0,说明当前存在写锁，返回 0
    return (((s = state) & WBIT) == 0L) ? (s & SBITS) : 0L;
}

// 验证
public boolean validate(long stamp) {
    U.loadFence();// 使用读屏障，禁止将之前的读取成员属性的操作重排序到后面的验证逻辑之后 

    // 将之前返回的 stamp 和当前 state 对比
    // SBITS = ~RBITS(127), SBITS的后7位为0，其他都为1
    // 一、stamp = 0，有写锁。 stamp & SBITS = 0, 分析state & SBITS
    //  1、当前有写锁，锁标志位肯定为 1 ，那么 state & SBITS 肯定不等于 0，最起码锁标志对应的那一位就肯定为1，state & SBITS ！= stamp & SBITS
    //  2、当前无锁，之前的写锁释放，版本号肯定 +1 ，不为 0。state & SBITS 肯定也不等0，state & SBITS ！= stamp & SBITS
    // 二、stamp，无写锁， stamp & SBITS 就是版本号 + 锁标志 0
    //  1、当前有锁，锁标志位肯定为 1 ，那么 state & SBITS 的最后一位锁标志是1，不相等
...
1. 位运算检查写锁是否被占用，读锁是否超限制
   * 满足条件，直接CAS 修改state值，并返回新的state值
2. 开始第一层大的自旋，第一层大的自旋里面包含了两种情况不同的自旋：
...
public long readLock() {
    ...
    //       1111 1111
    //           & 
    //       0111 1111  = 0111 1111 = 127 读模式 最多容纳 127 个线程  
    // 3、state = s + runit(1),也就是悲观读线程数 +1
    // 条件一：队列内无元素
    // 条件二：悲观读锁数量没有到达上限，最高能到 127，126时需要调用 tryIncReaderOverflow
    // 条件三：CAS 更新悲观读数量
    // 申请锁成功则返回 0，否则执行 acquireRead
...
private long acquireRead(boolean interruptible, long deadline) {
    ...
    for (int spins = -1;;) {
        WNode h;
        if ((h = whead) == (p = wtail)) { // 首尾节点相等，说明队列为空，有线程在排队不会进入 if
            // 前面没获取到锁，队列又为空，应该马上就是当前线程获取锁了
            // 第二次自旋 自旋 64 次 目的是为了看马上能不能获取锁（排队队列为空，没线程排队时，会在这里自旋获取锁）
            for (long m, s, ns;;) {
                // 这里是个三目运算，代码太长，拆开来看
                // (m = (s = state) & ABITS) < RFULL；和进入 readLock() 方法时的条件一样，判断读锁的数是否达到最大值，只有写锁被获取，这里就是 false
                // 我们假设前面写锁被获取了，现在获取读锁，m 就是128，大于 RFULL 126
                // 没有达到最大值，CAS 修改状态值 U.compareAndSwapLong(this, STATE, s, ns = s + RUNIT)
                // 超过最大值了，在 tryIncReaderOverflow 这累加   
                if ((m = (s = state) & ABITS) < RFULL ? U.compareAndSwapLong(this, STATE, s, ns = s + RUNIT) : (m < WBIT && (ns = tryIncReaderOverflow(s)) != 0L))
                    return ns;
                else if (m >= WBIT) {// if 条件成立，说明说明被占用
                    if (spins > 0) {
                        if (LockSupport.nextSecondarySeed() >= 0)
                            --spins; // 随机减自旋次数
                    } else {
                        if (spins == 0) { // 自旋次数减到 0 了，还没获取到读锁
                            WNode nh = whead, np = wtail;
                            if ((nh == h && np == p) || (h = nh) != (p = np))
                                break; // 退出自旋
                        } 
                        spins = SPINS; // 初始自旋次数 64 次
                    }
                }
            }
        ...
        if (p == null) { // 尾节点为空，初始化排队队列,有线程在排队时不会进入到这里
            ...
            if (U.compareAndSwapObject(this, WHEAD, null, hd)) // CAS 设置头节点
                // 运行到这里后，会回到第一次的自旋，再次进入到第二次自旋，这次 spins 为 0，只会自旋一次
                wtail = hd;
        } else if (node == null)
            ...
            // 上面那个 if 分支进不去，只有条件：队列不为空 and  尾节点是读模式 为真
            // 进入到了这里，说明 CAS 失败
            // 这里的 CAS 就是把当前节点加入到尾节点的 cowait 栈里面
            // 从这里可以看出加入的顺序是个栈结构，先把旧的尾节点的 cowait 赋值给 node 节点的 cowait,然后再把 node 节点赋值给尾节点
            node.cowait = null;
        else { 
            // 进入到这，说明上面的 if 分支都没有进去，尾节点不为空，当前节点不为空，队列不为空，尾节点是读模式，上面 CAS 修改成功
            ...
            for (;;) {
                ...
                if ((h = whead) != null && (c = h.cowait) != null &&
                    ...
                    // 头节点不为空，且其 cowait 节点不为空，唤醒整个 cowait 栈的线程
                    U.unpark(w);
                if (h == (pp = p.prev) || h == p || pp == null) {
                    ...
                    do {
                        ...
                        if ((m = (s = state) & ABITS) < RFULL ?  
                            ...
                            return ns;
                    } while (m < WBIT); // m < WBIT 时表示写锁没有被占用，一直尝试获取锁
                }
                if (whead == h && p.prev == pp) { // 对头没有发生变化 ，队尾也没发生变化
                    long time;
                    if (pp == null || h == p || p.status > 0) { // 队尾的前驱节点为空 或者 头节点等于尾节点 或者 老的尾节点被取消（>0 的状态只有 1，取消）
    ...
    for (int spins = -1;;) {
        ...
        if ((h = whead) == p) {  // 如果队列为空，说明马上轮到当前线程获得锁了
            // 这个大的 if 里面做的就是获取锁
            if (spins < 0)
                // 初始化本次自旋获取锁的次数：1024 次
                spins = HEAD_SPINS;
            else if (spins < MAX_HEAD_SPINS)
                // 上面 1024 次自旋没有获取到锁，就自旋翻倍：2048 次，继续下面的自旋
                spins <<= 1;
            // 开始自旋获取锁
            for (int k = spins;;) { // spin at head
                long m, s, ns;
                // 这个 if 条件 检查了是否可以获取锁，如果可以就 CAS 获取锁
                if ((m = (s = state) & ABITS) < RFULL ?
                    U.compareAndSwapLong(this, STATE, s, ns = s + RUNIT) :
                    (m < WBIT && (ns = tryIncReaderOverflow(s)) != 0L)) {
                    // 进入到这个 if 里面，说明就获得了锁
                    WNode c; Thread w;
                    // 这里的 node 节点是还没有绑定 thread 的
                    whead = node;
                    node.prev = null;
                    // 要唤醒当前 node 节点中的所有 cowait 节点线程
                    // 当前节点是在上面的第一层大的自旋入队的，其他获取读锁的节点都是挂在这个节点的 cowait 下的
        ...
        if (whead == h) { // 头节点没变过
            if ((np = node.prev) != p) { // 检查节点的链接关系
                ...
                // 检查尾节点的状态，为 0 则更新成 -1，回到第二层大的循环开始处
                U.compareAndSwapInt(p, WSTATUS, 0, WAITING);
            else if (ps == CANCELLED) { // p 节点被取消，删除这个节点
                if ((pp = p.prev) != null) {
                    node.prev = pp;
                    pp.next = node;
                }
            } else { // 上面 2048 次自旋后还是没获取到锁，进入到最终阻塞线程的环节
                long time;
                if (deadline == 0L)
                    time = 0L;
...
2. CAS 解锁并且更新state值
   1. 读锁数没有饱和溢出，更新完唤醒下一个节点
      * 检测队列中是否有排队获取锁的线程
        * 唤醒下一个等待获取锁的线程（`unpark(thread)`）
   2. 读锁数溢出，尝试减少 readerOverflow，更新完不唤醒下一个节点
...
public void unlockRead(long stamp) {
    ...
    for (;;) { // 自旋
        ...
        if (m < RFULL) {  // 锁标识小于读锁的最大标识
            if (U.compareAndSwapLong(this, STATE, s, s - RUNIT)) { // CAS 更新 state 值
                if (m == RUNIT && (h = whead) != null && h.status != 0) // 头结点不为空
                    release(h); // 唤醒下一个节点
                break;
            }
        } else if (tryDecReaderOverflow(s) != 0L) 
            // 读锁个数饱和溢出，尝试减少 readerOverflow
            break;
    }
}
...
public void unlockRead(long stamp) {
    ...
    for (;;) {
        ...
        // m = 126 ,ABITS =  255 ,则 state = 126
        //如果 readerOverflow != 0,并且 m = RFULL，将 state 设置为126
        else if (tryDecReaderOverflow(s) != 0L)
            break;
    }
}
...
private long tryDecReaderOverflow(long s) {
    //如果 ABITS = 255, s 的后八位为 126 , ABITS & s = RFULL 
    if ((s & ABITS) == RFULL) {
...
3. 开始第二层自旋（尝试获取锁，阻塞线程），第二层自旋和第三层自旋嵌套执行的：
   ...
   1. 如果头节点和之前的尾节点p还是是同一个（没有其他获取锁的节点排队已经入队）, 说明马上应该轮到node节点获得锁（排队的只有node节点）
      ...
      * 初始化第三层自旋次数（第一次1024，第二次2048），开启第三层自旋
        ...
        位运算检查锁是否有被释放（`(s = state) & ABITS) == 0L`），CAS 修改 state 值，修改成功，退出，返回新的state值
   ...
   3. 第三层自旋完成后还是没有获取到锁，阻塞当前线程，等待被唤醒，被唤醒后继续第二层自旋获取锁，重复这个过程，直到获取锁成功退出

```java
 public long writeLock() {
     long s, next;  // bypass acquireWrite in fully unlocked case only
     // 条件一：用来表示读锁和写锁是否可以被获取
     // 第一次时，state 初始值是256，ABITS是255，计算过程：0001 0000 0000 && 0000 1111 1111 ，结算结果为0

     // 条件二：执行CAS操作
     // STATE 是 state 字段的内存地址相对于此对象的内存地址的偏移量，s 是期望值， next = s + WBIT 是更新值；s + WBIT 也就是 256 + 128，next计算结果为384，用二进制位表示就是 0001 1000 0000，也就是将第 8 位设置为 1，就是获得写锁。
     // 如果 CAS 更新成功，返回 next 值，成功获得写锁
     // 如果 CAS 执行失败，则执行 acquireWrite ，进入等待队列获取锁
...
private long acquireWrite(boolean interruptible, long deadline) {
    ...
    for (int spins = -1;;) { // spin while enqueuing
        long m, s, ns;
        // 这个 if 和外面一样的，从代码运行到这期间所有没有被释放
        if ((m = (s = state) & ABITS) == 0L) {
            // CAS 再次尝试获取下写锁
            if (U.compareAndSwapLong(this, STATE, s, ns = s + WBIT))
                // 成功获取写锁
                return ns;
        }  else if (spins < 0)  // 走到这，说明上面还是没获取到写锁，写锁被占用了，m 的值为128
            // 1.确定自旋的次数 spins: 64 or 0 
            spins = (m == WBIT && wtail == whead) ? SPINS : 0;
        else if (spins > 0) {
            // 2.自旋的次数大于 0，随机减一次自旋次数，直到减到 spins 为 0（这里实际是空转，没什么特点的逻辑处理）
            if (LockSupport.nextSecondarySeed() >= 0)
                --spins;
        } else if ((p = wtail) == null) { // initialize queue
            // 3.自旋 spins 减到 0 后会立马执行到这里
            // p 被赋值为尾节点  
    ...
    for (int spins = -1;;) {
        WNode h, np, pp; int ps; 
        if ((h = whead) == p) { // 如果头节点和之前的尾节点 p 是同一个, 说明马上应该轮到 node 节点获得锁
            if (spins < 0)
                // 1.设置自旋次数  1024 or 0
                spins = HEAD_SPINS;
            else if (spins < MAX_HEAD_SPINS)
                // 第一次自旋 1024 次没有获取到锁，这次自旋翻倍
                // 自旋次数*2  2048 继续进入到下面的自旋
                spins <<= 1;
            for (int k = spins;;) { // spin at head
                // 2.第三次自旋,不断尝试获得锁(自旋1024或者2048次)，直到成功获得锁 或者 break
                long s, ns;
        ...
        if (whead == h) { // 如果头结点没有变化
            ...
            if ((np = node.prev) != p) { 
                ...
                // 3.上面第三次自旋 break 后会进入到这里，修改尾节点状态
                // 更新尾节点的状态为 WAITING：-1， 然后继续回到第二次自旋的地方，重新开始自旋
                U.compareAndSwapInt(p, WSTATUS, 0, WAITING);
                ...
                if (deadline == 0L)
                    ...
                    // 设置了超时时间 已经超时，取消当前 node 节点
                    return cancelWaiter(node, node, false);
                Thread wt = Thread.currentThread();
                // 为给定地址设置值，忽略修饰限定符的访问限制，与此类似操作还有: putInt，putDouble，putLong，putChar等
                // 将当前锁实例（this）设置为线程的阻塞原因（parkBlocker），以便在调试时能够更清晰地了解线程被阻塞的原因
                U.putObject(wt, PARKBLOCKER, this);
                // node 节点指向当前线程
                node.thread = wt;
                // p.status < 0 的只有-1 也就是 WAITING 
                if (p.status < 0 && (p != h || (state & ABITS) != 0L) && whead == h && node.prev == p)
                    // 阻塞当前线程
                    U.park(false, time);  // emulate LockSupport.park
                // 线程被唤醒后，清除节点的线程
                node.thread = null;
                // 将线程的阻塞原因（parkBlocker）清除，避免内存泄漏或误报
                U.putObject(wt, PARKBLOCKER, null);
...
1. 检查锁印章戳是否匹配，锁是否有被占用，检查不通过抛出异常
2. 通过位运算`stamp += WBIT`计算新的state值，state 二进制位的第8位会被设置成0就是写锁解锁
3. 检测队列中是否有排队获取锁的线程
   * 唤醒下一个等待获取锁的线程（`unpark(thread)`）
...
public void unlockWrite(long stamp) {
    ...
    // (stamp & WBIT) == 0L 为 true 的话说明锁没有被占用
    if (state != stamp || (stamp & WBIT) == 0L)
        // 抛出异常
        throw new IllegalMonitorStateException();
    // 释放写锁，会增加 state 的版本
    // stamp += WBIT 等于二进制（第一次加写锁和解锁） 0001 1000 0000 + 0000 1000 0000 == 0010 0000 0000
    // 解锁会把 stamp 的二进制第 8 位设置为 0
    // 相当于重新赋值 state 值
    state = (stamp += WBIT) == 0L ? ORIGIN : stamp;
    if ((h = whead) != null && h.status != 0) // 头结点不为 && 状态不为初始状态0（一般是 WAITING -1），说明队列中有排队获取锁的线程
        // 唤醒头节点的后继节点
        release(h);
}
```
...
参考网址：[](https://blog.csdn.net/qq_40276626/article/details/119873114)

参考网址：[](https://www.cnblogs.com/admol/p/14007975.html)
...
2. CountDownLatch 使用给定的计数值（count）初始化

   * await 方法会阻塞直到当前的计数值被 countDown 方法的调用达到 0，count 为 0 之后所有等待的线程都会被释放，并且随后对await 方法的调用都会立即返回

     > 所有阻塞线程进入 AQS 队列，等待 count 为 0 后全部依次唤醒
     >
     > 这是一个一次性现象 —— count 不会被重置。需要重置 count -> 考虑使用 CyclicBarrier
    ...
    for (int j = 0; j < 10; j++) {
        ...
        service.submit(() -> {
            for (int i = 0; i <= 100; i++) {
                ...
                all[finalJ] = i + "%";
                System.out.print("\r" + Arrays.toString(all));	// \r 代表覆盖
            }
            latch.countDown();
        });
    }
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-CyclicBarrier工作原理.png" style="zoom:80%;" />
...
* Semaphore 的 permits（state）为 3，这时 5 个线程来获取资源
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-Semaphore工作流程1.png" style="zoom:80%;" />
...
* 这时 Thread-4 释放了 permits，状态如下
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-Semaphore工作流程2.png" style="zoom:80%;" />
...
> 设计思想和LongAdder很相似，都是无锁 + 分散热点，在多槽位交换时手动实现避免伪共享
...
> `bound`：bound 的高24位相当是版本号，每添加或删除一个 node 元素，bound 的高 24 位都将加 1；而 bound 的低 8 位表示arena 数组的最大序数索引(`p.index`的最大值)，添加一个 Node 元素，低 8 位加 1，删除一个 Node 元素，低 8 位减 1
>
> `ASHIFT`：为了防止伪共享，arena中的元素不是连续分布的，每个元素间隔（1 << ASHIFT）个下标长度，假设一个 node 元素的有效索引为 i，那么（i << ASHIFT）才是实际的数组下标



```java
// node 数组，存储每个线程待交换数据的相关信息
private volatile Node[] arena;
// 封装两个线程时的数据交换信息
private volatile Node slot;
// 高 24 位是版本号，低 8 位是 arena 数组的最大有效索引(元素为 null 不计数为有效索引)
private volatile int bound; 

// arena 每个元素之间的间隔位移 255(1<<7)   
private static final int ASHIFT = 7;
// 数组 arena 的最大容量
private static final int MMASK = 0xff;
// bound 的版本号，bound 每次修改一次，其二进制的倒数第 9 位加 1  
private static final int SEQ = MMASK + 1;//二进制 0b1 0000 0000

// cpu 个数，用来确定自旋次数
private static final int NCPU = Runtime.getRuntime().availableProcessors();

// arena 的最大下标，表示所有线程无竞争或 arena 的最大可用的下标
...
```java
// Participant 继承了 ThreadLocal，用来保存线程本地变量 Node
...
```java
// Node 理解为线程自身携带的要交换的数据节点对象
@jdk.internal.vm.annotation.Contended static final class Node {
    // 以下字段，多槽交换时使用
    int index;              // Arena 多槽数组的索引
    int bound;              // 记录上次的 bound
    int collides;           // 当前 bound 的 CAS 失败次数

    // 以下字段，单槽交换时使用
    int hash;               // 线程的伪随机数，用于自旋优化
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-Exchange数据交换方式.png" style="zoom:80%;" />

```java
// Exchanger 只有两个重载的公共方法 exchange(V) 、exchange(V, long, TimeUnit)，前者不限定阻塞时长，而后者要限定阻塞时长（超时版本），以 exchange(V) 为例进行说明
...
```java
for (;;) {
    if (slot is empty) {// offer 第一个线程执行 exchange，添加 slot 阻塞等待
        // slot 是空的，就将构造一个 Node 节点，（将待交换值 item 包装成一个 Node）
        place item in a Node;

        if (can CAS slot from empty to node) { //使用 CAS 成功将 slot 设为刚构造的 node 结点
            // 当前线程休眠等待，直到被唤醒
            wait for release;
            // 被唤醒后返回 node 中匹配的数据 slot.match
            return matching item in node;
        }
    }
    // slot 不为空，CAS 成功将 slot 中设为 null
    else if (can CAS slot from node to empty) { // release 第二个线程执行 exchange，获取 slot 中的值，唤醒等待线程
        // 获取 slot 中的 item,（这个 item 是第二个线程要返回的值）
        get the item in node;
        // 将待交换的 item 设置到 slot.match 中
...
2. 然后，线程会自旋一段时间，如果经过一段时间的自旋还是等不到配对线程到达，就会进入阻塞。（**这里之所以不直接阻塞，而是自旋，是出于线程上下文切换开销的考虑，属于一种优化手段，即 `spin -> yield -> block`**）

* **稍后到达的配对线程：** 如果当前线程（配对线程）不是首个到达的线程，则到达时槽（**slot**）已经被占用，此时 **slot** 指向首个到达线程自身的 **Node** 结点。配对线程会将 **slot** 置空，并取Node中的 **item** 作为交换得到的数据返回，另外，配对线程会把自身携带的数据存入 **Node** 的 **match** 字段中，并唤醒  `Node.parked` 所指向的线程（也就是先到达的线程）

  > **首先到达的线程被唤醒：** 线程被唤醒后，由于 **match** 不为空（存放了配对线程携带过来的数据），所以会退出自旋，然后将 **match** 对应的值返回

<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-Exchange单槽位交换.png" style="zoom:80%;" />

```java
private final Object slotExchange(Object item, boolean timed, long ns) {
    Node p = participant.get();
    Thread t = Thread.currentThread();
    // 外部方法 exchange 的 if 语句中的 Thread.interrupted() 能重置中断，将其设为非中断的，外部的 exchange 方法将抛出中断异常
    if (t.isInterrupted()) // preserve interrupt status so caller can recheck 
        return null;

    // 第一次自旋，初始化 slot 和 arena
    for (Node q;;) {
        // q 代表当前的slot
        if ((q = slot) != null) { // slot不为空，
            // CAS 将 slot 设空，
            // 获取并返回对方线程传入的交换数据 q.item
            // 将当前线程待交换数据 item 设到匹配项 q.match
            // 唤醒对方线程（对方线程在被唤醒后将返回 q.match）
            ...
            // CAS 失败，有线程竞争，初始化 arena
            // create arena on contention, but continue until slot null
            if (NCPU > 1 && bound == 0 &&
                U.compareAndSwapInt(this, BOUND, 0, SEQ))// 将 bound 的倒数第 9 设为 1，bound 低 8 位全是零
                arena = new Node[(FULL + 2) << ASHIFT];
        }
        else if (arena != null)
            // slot 为空和 arena 不为空，返回空，然后将执行 arenaExchange 方法
            return null; // caller must reroute to arenaExchange
        else { // slot 和 arean 均为空，第一个线程首次执行 exchange 方法，就是这种情况
            // 将待交换的数据放在当前线程变量 p 中，
            p.item = item;
            // CAS 设值
            if (U.compareAndSwapObject(this, SLOT, null, p))
                break;// CAS 成功了，进入下面的自旋等待
            // CAS 失败，可能其他线程先于当前线程 CAS 设值成功，可以获取另外其他线程传的 item.
            // 重新将 p.item 设为 null，为下次自旋准备
            p.item = null;
        }
    }
    ...
    int spins = (NCPU > 1) ? SPINS : 1;// CPU 数目只有一个时，不进行自旋，防止浪费有限的 CPU 资源
    Object v;
    while ((v = p.match) == null) {// 反复获取 p.match，v 不为空才返回
        if (spins > 0) {
            h ^= h << 1; h ^= h >>> 3; h ^= h << 10;// 异或操作将 h 更随机
            if (h == 0)
                // p.hash 作为成员变量，初始值为 0,将其初始化为非零的值
                h = SPINS | (int)t.getId();
            else if (h < 0 && (--spins & ((SPINS >>> 1) - 1)) == 0) // 将自旋次数减 1
                // 礼让其他线程
                Thread.yield();
        }
        else if (slot != p)// slot 与 p 不等，表明 slot 被其他给修改了，需要重新自旋，重新最新的值
            spins = SPINS;
        // 自旋次数为 0，不再自旋，准备休眠等待
        else if (!t.isInterrupted() && arena == null &&  // 未中断，arena 不为空
                 (!timed || (ns = end - System.nanoTime()) > 0L)) {// 未超时
            U.putObject(t, BLOCKER, this); // 记录当前线程阻塞的对象 this
            p.parked = t;// 阻塞的线程是当前线程 t
            if (slot == p)
                U.park(false, ns);// 当前线程休眠
            // 被唤醒后，将阻塞相关的属性重新设为 null
            p.parked = null;
            U.putObject(t, BLOCKER, null);
        }
        // arena 为 null 或超时了或中断了（非正常状态），尝试将 slot 设为 null（方法准备返回了）
...
> 多槽位交换（并发量高时选用）
>
> 在单槽交换中，同时出现了多个配对线程竞争修改 slot 槽位，导致某个线程 CAS 修改 slot 失败时，就会初始化 arena 多槽数组，后续所有的交换都会走 arenaExchange

**伪共享**：手动实现避免伪共享

问题：在定位**arena**数组的有效槽位时，需要考虑缓存行的影响。由于高速缓存与内存之间是以缓存行为单位交换数据的，根据局部性原理，相邻地址空间的数据会被加载到高速缓存的同一个数据块上（缓存行），而数组是连续的（逻辑，涉及到虚拟内存）内存地址空间，因此，多个**slot**会被加载到同一个缓存行上，当一个**slot**改变时，会导致这个**slot**所在的缓存行上所有的数据（包括其他的**slot**）无效，需要从内存重新加载，影响性能

解决：为了防止伪共享，arena中的元素不是连续分布的，每个元素间隔（1 << ASHIFT）个下标长度，假设一个 node 元素的有效索引为 i，那么（i << ASHIFT）才是实际的数组下标
...
private final Object arenaExchange(Object item, boolean timed, long ns) {
    ...
    for (int i = p.index;;) {                      // access slot at i
        ...
        // 取出对应的 node 节点，
        // (1 << ASHIFT) 是每个 node 的索引间隔（防止伪共享）
        Node q = (Node)U.getObjectVolatile(a, j = (i << ASHIFT) + ABASE);
        // 因为要即将取走这个 node 中的 item，所以 CAS 将 arena 中第 i 个有效元素设为 null
        if (q != null && U.compareAndSwapObject(a, j, q, null)) {
            ...
            q.match = item;// 设置匹配 item
            Thread w = q.parked;
            if (w != null)
                U.unpark(w);// 唤醒对方线程
            return v;
        }
        // (bound) & MMAS 取 bound 的低8位，所以 m 最大索引
        // q 为 null 且索引位 i 小于等于最大索引
        else if (i <= (m = (b = bound) & MMASK) && q == null) {
            p.item = item;                         // offer
            // 将 CAS 设置 arena 数组中对应位置的 node
            if (U.compareAndSwapObject(a, j, null, p)) {
                ...
                for (int h = p.hash, spins = SPINS;;) {
                    // 返回对方传入匹配数据 p.match，并将相应的数据清空
                    Object v = p.match;
                    if (v != null) { // 有匹配的数据
                        U.putOrderedObject(p, MATCH, null);// 取出 match 后，将之清空
                        // 将 p.item 清空，为下次做准备
                        p.item = null;             // clear for next use
                        p.hash = h;// 记录 hash
                        return v;
                    }
                    else if (spins > 0) { // v 为空，且还有剩余自旋次数
                        // 异或操作将 h 更随机
                    ...
                    // 其他情况，如中断了，m 不为 0，或超时了 （非正常状态）
                    else if (U.getObjectVolatile(a, j) == p &&
                             U.compareAndSwapObject(a, j, p, null)) {
                        if (m != 0)                // try to shrink
                            // bound 加一个 SEQ 单位，-1 表示最大有效索引减 1，arena 中少了一个元素
            ...
            else// CAS 设置 arena 数组中对应位置的 node 失败
                // 清空 p.item，将自旋重试
                p.item = null;                     // clear offer
        }
        else {  // q 不为空，i 大于最大的有效索引
            if (p.bound != b) { // stale; reset 值 b 过期了，重新获取
                p.bound = b;
                p.collides = 0;// CAS 失败次数为 0
                i = (i != m || m == 0) ? m : m - 1;// i 取最大可用的索引（准备从尾向前遍历）
            }
            else if ((c = p.collides) < m || m == FULL ||
                     // +1 表示最大有效索引加 1，arena 中多了一个元素
                     !U.compareAndSwapInt(this, BOUND, b, b + SEQ + 1)) {
                // 失败次数加 1
                p.collides = c + 1;
...
参考网址：[](https://www.cnblogs.com/gocode/p/analysis-source-code-of-Exchanger.html)

参考网址：[](https://mp.weixin.qq.com/s?__biz=MzIwMjU4MzU4MA==&mid=2247484163&idx=1&sn=b26198657eac5f3616e690028df11b7d&chksm=96dd3870a1aab166e4811a63fd08d8aff61995aec52898cf053711949711749241daa7303a41&scene=21#wechat_redirect)
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/ConcurrentHashMap数据结构.png" style="zoom:80%;" />
...
public class ConcurrentHashMapDemo{
    ...
    public static void main(String[] args){
        ...
        System.out.println("Map大小：" + map.size());// 20 万
    }
}
...
* **链表成环**
  ...
  > 线程1迁移完毕，线程2执行完e和next赋值后CPU时间片用完阻塞

  * 操作并未加锁
  * 全部采取头插法，即迁移前后数据顺序刚好相反
  * 直接对原节点进行操作，即两个线程局部变量e和next指向的对象是同一个，出现线程1e和next指向的对象实际已经是线程2迁移完的对象顺序



JDK 8 虽然将扩容算法做了调整但仍不意味着能够在多线程环境下能够安全扩容，还会出现其它问题（如扩容丢数据）

**Java 1.8 0_202版本**：具体参见源码解析部分

* 增(putVal)删改(replaceNode)操作时全部已针对Table数组元素(链表头节点或红黑树根节点)加锁，即不存在线程安全
* 读(get)操作没有加锁，可能存在扩容时读取丢失数据现象
* putVal增加链表新节点时使用尾插法，扩容迁移时链表采取的是lastRun优化+ 新建节点头插法
* 可能出现问题情况：线程1在进行扩容迁移，由于扩容迁移顺序是lastRun优化 -> 新建节点头插法 -> 更新fwd至旧Table，线程1刚进行完lastRun优化，即把lastRun部分节点直接迁移到nextTable中(这次迁移直接移动原节点，并没有新建节点)，但是迁移没完成，fwd没有更新至旧Table，此时线程2来进行lastRun部分节点的数据读取就会出现数据丢失的现象，不过由于链表节点数不超过8个，这次迁移的速度相对较快，即数据丢失状态持续时间较短，等fwd更新完就能正常查询这部分数据(数据并未丢失，只是已迁移至nextTable)
...
* 有参构造：
  ...
  private static final int tableSizeFor(int c) {
      // 把最高位是 1 的位以及右边的位全部置 1
      int n = c - 1; // 避免 n 初始即为 2 的整数次幂
      n |= n >>> 1; // 从高至低按指数速度进行置 1，第一次改变一个，第二次改变两个，第三次改变四个，第四次改变八个，第五次改变十六个，此时已经总数有三十二个了，已经超出默认最大容量
      n |= n >>> 2;
      n |= n >>> 4;
      n |= n >>> 8;
      n |= n >>> 16;
      ...
      if (check >= 0) {
          ...
          while (s >= (long)(sc = sizeCtl) && (tab = table) != null &&
              ...
              if (sc < 0) {
                  ...
                  // 勘误两个条件：
                  // 条件二：sc == (rs << 16) + 1，true 代表扩容完成，因为低 16 位是 1 代表没有线程扩容了
                  // 条件三：sc == (rs << 16) + MAX_RESIZERS，判断是否已经超过最大允许的并发扩容线程数
...
* `transfer()`：数据转移到新表中，完成扩容
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ConcurrentHashMap-LastRun机制.png" style="zoom:80%;" />
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-CopyOnWriteArrayList弱一致性.png" style="zoom:80%;" />
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ConcurrentSkipListMap数据结构.png" style="zoom:80%;" />
...
* `findPredecessor()`：寻找前置节点
  ...
  > **作用**：返回一个键值严格小于给定键的基础层节点，如果没有这样的节点，则返回基础层的头节点。同时，还会取消沿途找到的指向已删除节点的索引链接
  >
  > 
  >
  > **调用者依赖于这种清除指向已删除节点索引的副作用**
  >
  > 
  >
  > **工作流程**：从最上层的头索引开始向右查找（链表的后续索引），如果后续索引的节点的 key 大于要查找的 key，则头索引移到下层链表，在下层链表查找，以此反复，一直查找到没有下层的分层索引为止，返回该索引的节点。如果后续索引的节点的 key 小于要查找的 key，则在该层链表中向后查找。由于查找的 key 可能永远大于索引节点的 key，所以只能找到目标的前置索引节点。如果遇到空值索引的存在，通过 CAS 来断开索引
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ConcurrentSkipListMap-Put流程.png" style="zoom:80%;" />
...
* `findNode()`
  ...
  > 
  >
  > **作用**：返回持有键的节点，如果没有这样的节点，则返回null，同时清除沿途遇到的任何已删除的节点。通过从findPredecessor返回的前驱节点开始，在基础层反复遍历以查找键，并在遇到基础层的删除操作时进行处理。某些调用者依赖于这种清除已删除节点的副作用
  >
  > 
  >
  > **在以节点n为中心的遍历步骤中，如果出现以下情况，则会重新启动**：
  >
  > (1) 在读取n的next字段后，n不再是假设的前驱节点b的当前后继节点，这意味着我们没有一个一致的3节点快照，因此无法取消后续遇到的已删除节点的链接
  >
  > (2) n的value字段为null，表明n已被删除，在这种情况下，我们会协助正在进行的结构删除操作，然后重试。尽管存在一些无需重启即可取消链接的情况，但这些情况在此并未被区分，因为通常情况下，进行这种区分的成本会高于重启的成本
  >
  > (3) n是一个标记节点，或者n的前驱节点的value字段为null，这表明（在其他可能性中）findPredecessor返回了一个已删除的节点。我们无法取消该节点的链接，因为我们不知道它的前驱节点，因此依赖于findPredecessor的另一次调用来注意到并返回某个更早的前驱节点，而它确实会这样做。虽然这种检查实际上只需要在循环的开始处进行（并且b的value检查根本不需要），但为了帮助避免线程之间的竞争，每次迭代都会进行检查，因为调用者可能会因为无法更改链接而失败，因此无论如何都会重试。doPut、doRemove和findNear中的遍历循环都包含了这三种类型的检查，而findFirst和findLast及其变体中也有专门的版本。由于每个版本都依赖于以特定顺序执行的局部变量字段的读取，因此它们无法轻松共享代码
  ...
  private Node<K,V> findNode(Object key) {
      ...
      outer: for (;;) {
          ...
          for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {
              ...
              if (b.value == null || v == n)
                  break;
               // 6.如果要获取的 key 与后续节点的 key 相等，返回节点
              if ((c = cpr(cmp, key, n.key)) == 0) {
                  return n;
              }
              // 7.key < n.key，因为 key > b.key，b 和 n 相连，说明不存在该节点或者被其他线程删除了
              if (c < 0)
                  break outer;
              b = n;
              n = f;
...
* `remove()`
  ...
  <img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ConcurrentSkipListMap-remove流程.png" style="zoom:80%;" />
...
### NoBlocking -> **ConcurrentLinkedQueue**
...
**ConcurrentLinkedQueue 线程安全问题**：优先保证入队出队的线程安全

> 入队(出队)的操作实质都是casNext(casItem)，无论从哪个位置开始并发入队(出队)，都可以循环 CAS，直到入队(出队)成功
...
public boolean offer(E e) {
    ...
    for (Node<E> t = tail, p = t;;) {
        ...
        if (q == null) {
            ...
            // 出队可能会调用 updateHead 方法，将节点 next 指向自己，方便 GC 回收，即该节点被移除，如果此时刚好 tail 也指向该节点就会出现 p == q 的情况
            // 当 tail 不指向最后节点时，如果执行出队操作，可能将 tail 也移除，tail 不在链表中 
        	// 此时需要对 tail 节点进行复位，复位到 head 节点
            p = (t != (t = tail)) ? t : head;
        else
            // 推动 tail 尾节点往队尾移动
            // 这里和 poll 方法不一样，但效果一致，即这里不通过 continue 去更新获取 tail
            p = (p != t && t != (t = tail)) ? t : q;
    }
}
```
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ConcurrentLinkedQueue入队操作1.png" style="zoom:80%;" />

<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ConcurrentLinkedQueue入队操作2.png" style="zoom:80%;" />

<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ConcurrentLinkedQueue入队操作3.png" style="zoom:80%;" />
...
> 和入队更新 tail 原理一致，当 head 节点和头节点的距离 **大于等于 1** 时（每出队两次）更新 head，可以减少使用 CAS 更新 head 节点的消耗，从而提高出队效率，因此 head 节点具有**滞后性**
...
public E poll() {
    ...
    for (;;) {
        ...
        for (Node<E> h = head, p = h, q;;) {
            ...
            // 如果两个线程并发来出队，可能出现 head 数据过时，比如线程 1 执行 h = head, p = h 后由于 CPU 时间片使用完陷入阻塞，线程 2 成功出队并且进行 updateHead 更新 head，使得线程 1 的 p 的 next 指向自己，就出现 p == q 的情况，此时需要重新更新获取 head
            else if (p == q)
...
<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ConcurrentLinkedQueue出队操作1.png" style="zoom:80%;" />

<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ConcurrentLinkedQueue出队操作2.png" style="zoom:80%;" />

<img src="https://gitee.com/wwwiser/typora-image/raw/master/JUC-ConcurrentLinkedQueue出队操作3.png" style="zoom:80%;" />
...
* `peek()`：会改变 head 指向，执行 peek() 方法后 head 会指向第一个具有非空元素的节点
  ...
  > remove() 方法调用过多后，导致 head 节点过时，可以通过 peek() 进行更新
...
* `remove()`：移除元素
  ...
  > * 该方法的 CAS 操作没有通过自旋进行，即无法保证 remove 方法有效，有效性通过两次 CAS 操作保证(第一次是打上标记，第二次是直接移除)
  >
  > * 返回false，不代表此次移除操作无效，可能`p.casItem(item, null)`失败，`pred.casNext(p, next)`成功，即元素没修改直接被移除队列
  >
  > * 如果存在重复元素，CAS 操作的顺序按照队列从前向后，直至 CAS 操作成功一次，如果都没成功，此次 remove 方法失效
  >
  > * 如果 remove 方法有效，可能会移除队列中一个元素(如果有重复元素，移除的元素位置不一定，看 CAS 操作在哪个位置成功)，也可能会移除多个元素(可能`p.casItem(item, null)`失败，`pred.casNext(p, next)`成功)
  >
  > * 删除的节点不一定会在此次 remove 方法内被移除队列，如果`p.casItem(item, null)`成功，`pred.casNext(p, next)`失败，则该节点不会被移除队列，仅仅是 item 被赋值成 null，但是该节点会在后续出队操作中通过更新 head 被 GC 回收
      ...
      if (o != null) {
          ...
          for (Node<E> p = first(); p != null; pred = p, p = next) {
              ...
              if (item != null) {
                  ...
                  removed = p.casItem(item, null);// 打上标记的 CAS 操作
              }
              // 获取删除节点的后继节点
              next = succ(p);
              // 将被删除的节点移除队列
              if (pred != null && next != null) // unlink
                  pred.casNext(p, next);// 直接移除的 CAS 操作
              if (removed)
                  return true;
          }
      }
